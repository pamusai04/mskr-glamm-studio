...
import { updateHeroImage, fetchHeroImages } from '../../redux/slices/heroImagesSlice';

const MAX_FILE_SIZE = 5 * 1024 * 1024;
const ACCEPTED_IMAGE_TYPES = ['image/jpeg', 'image/jpg', 'image/png', 'image/webp'];

const EditHeroImagePage = () => {
  const { id } = useParams();
  ...
  }, [dispatch, heroImages.length]);

  useEffect(() => {
    const image = heroImages.find(img => img._id === id);
    if (image) {
      setNameOfTheImage(image.nameOfTheImage);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await dispatch(updateHeroImage({ id, data: { nameOfTheImage, image: imageFile } })).unwrap();
        ...
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          <div className="p-4 sm:p-6 md:p-8">
            <div className="mb-5 sm:mb-6">
              <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-800">Edit Hero Image</h1>
              <p className="text-xs sm:text-sm text-gray-500 mt-1">Update your hero image details</p>
            </div>
                  ...
                  {imagePreview ? (
                    ...
                    <div className="space-y-2 text-center">
                      ...
                      <p className="text-[10px] sm:text-xs text-gray-400">
                        Leave empty to keep current image • PNG, JPG, JPEG, WEBP • Max 5MB
                      </p>
                    </div>
                  )}
                </div>
...
export default EditHeroImagePage;