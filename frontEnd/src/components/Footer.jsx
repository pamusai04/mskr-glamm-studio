...
import { Instagram, MessageCircle, Home, Sparkles, Image, Info, Phone, Calendar, Star, MapPin, Mail, Clock, ShoppingBag, User, ChevronRight, Youtube, Navigation } from 'lucide-react';
...
const Footer = () => {
  ...
  const cleanPhoneNumber = currentMeta.phoneNumber?.replace(/[^0-9]/g, '');
  ...
  const accountLinks = [
    ...
    { label: 'Cart', path: '/cart', icon: ShoppingBag, badge: cart.length },
  ];

  const getGoogleMapsUrl = () => {
    const location = currentMeta?.location;
    if (location?.lat && location?.lng) {
      return `https://www.google.com/maps/dir/?api=1&destination=${location.lat},${location.lng}`;
    }
    return `https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(currentMeta?.location?.address || defaultMeta.location.address)}`;
  };
    ...
    <footer className="bg-gray-50 border-t border-gray-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div>
            <h4 className="text-gray-800 font-semibold text-sm mb-4">Quick Links</h4>
              ...
              {accountLinks.map((item) => {
                ...
                return (
                  <li key={item.label}>
                    <button
                      onClick={() => handleNav(item.path)}
                      className="flex items-center w-full text-gray-500 hover:text-pink-500 text-sm transition-colors group"
                    >
          ...
          <div>
            ...
            <div className="space-y-3 ">
              ...
              <div className="flex gap-3 items-center group cursor-pointer pt-2 border-t border-gray-100">
                ...
                <div>
                  <p className="text-gray-500 text-sm group-hover:text-pink-500 transition-colors">
                    {currentMeta.timeSlots?.[0]?.startTime} - {currentMeta.timeSlots?.[currentMeta.timeSlots.length - 1]?.endTime}
                  </p>
